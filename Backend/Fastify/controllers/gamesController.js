...
export async function getUserGames(request, reply) {
	...
	try {
		...
		if (!infos) {
			log.warn('Games access denied: Unauthorized request')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		let user = infos.user
		if (!user) {
			log.warn('Games access denied: User not found in token')
			return reply.code(401).send({ error: "User not found" })
		}
		if (!infos.accessToken) {
			log.warn('Games access denied: Missing access token')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		if (username) {
			user = usersModel.getUserByUsername(username)
			log.info(`Retrieving games for specific user: ${username}`)
		}
		const games = gamesModel.getUserGames(user.userId)
		log.success(`Games retrieved for user: ${user.username} (${games.length} found)`)
		return reply.send({ success: true, user: user, games: games, accessToken: infos.accessToken })
	} catch (err) {
		log.error(`Error retrieving user games: ${err.message}`)
...
export async function create1v1Game(request, reply) {
	...
	try {
		if (!player1 || !player2 || score_left === undefined || score_right === undefined) {
			log.warn('1v1 game creation failed: Missing parameters')
			return reply.code(400).send({ success: false, error: "Missing parameters" })
		}

		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('1v1 game creation denied: Unauthorized request')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		const user = infos.user
		if (!user) {
			log.warn('1v1 game creation failed: User not found in token')
			return reply.code(401).send({ error: "User not found" })
		}
		if (!infos.accessToken) {
			log.warn('1v1 game creation denied: Missing access token')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		...
		if (score_left < score_right) {
			...
			log.success(`1v1 game result: ${player2} wins against ${player1}`)
		} else {
			usersModel.updateGamesWon(player1User.userId)
			usersModel.updateGamesLost(player2User.userId)
			log.success(`1v1 game result: ${player1} wins against ${player2}`)
		}

		log.success(`1v1 game created successfully: ${player1} vs ${player2}`)
		...
		log.error(`Error creating 1v1 game: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
export async function create2v2Game(request, reply) {
	...
	try {
		if (!player1 || !player2 || !player3 || !player4 || score_left === undefined || score_right === undefined) {
			log.warn('2v2 game creation failed: Missing parameters')
			return reply.code(400).send({ success: false, error: "Missing parameters" })
		}

		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('2v2 game creation denied: Unauthorized request')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		const user = infos.user
		if (!user) {
			log.warn('2v2 game creation failed: User not found in token')
			return reply.code(401).send({ error: "User not found" })
		}
		if (!infos.accessToken) {
			log.warn('2v2 game creation denied: Missing access token')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		...
		if (!player1User) {
			log.warn(`2v2 game creation failed: Player1 '${player1}' not found`)
			return reply.code(404).send({ success: false, error: `Player '${player1}' not found`, accessToken: infos.accessToken })
		}
		if (!player2User) {
			log.warn(`2v2 game creation failed: Player2 '${player2}' not found`)
			return reply.code(404).send({ success: false, error: `Player '${player2}' not found`, accessToken: infos.accessToken })
		}
		if (!player3User) {
			log.warn(`2v2 game creation failed: Player3 '${player3}' not found`)
			return reply.code(404).send({ success: false, error: `Player '${player3}' not found`, accessToken: infos.accessToken })
		}
		if (!player4User) {
			log.warn(`2v2 game creation failed: Player4 '${player4}' not found`)
			return reply.code(404).send({ success: false, error: `Player '${player4}' not found`, accessToken: infos.accessToken })
		}

		log.info(`Creating 2v2 game: ${player1} & ${player2} (${score_left}) vs ${player3} & ${player4} (${score_right})`)
		...
		if (score_left < score_right) {
			...
			log.success(`2v2 game result: ${player3} & ${player4} win against ${player1} & ${player2}`)
		} else {
			usersModel.updateGamesWon(player1User.userId)
			usersModel.updateGamesWon(player2User.userId)
			usersModel.updateGamesLost(player3User.userId)
			usersModel.updateGamesLost(player4User.userId)
			log.success(`2v2 game result: ${player1} & ${player2} win against ${player3} & ${player4}`)
		}

		log.success(`2v2 game created successfully: ${player1} & ${player2} vs ${player3} & ${player4}`)
		return reply.code(201).send({ 
			success: true,
			message: "Game finished successfully",
			accessToken: infos.accessToken
		...
		log.error(`Error creating 2v2 game: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
