...
export async function login2v2(request, reply) {
	...
	try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			fastify.log.warn('2v2 login denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}
		...
		const accessToken = infos.accessToken
		if (!user || !accessToken) {
			fastify.log.warn('2v2 login denied: Invalid user or token')
			return reply.code(401).send({ error: 'Unauthorized' })
		}

		fastify.log.info(`2v2 game login attempt: Player 1: ${user.username}`)
