...
export async function googleConfig(request, reply) {
	try {
		log.debug(`Google config requested - Client ID: ${process.env.GOOGLE_CLIENT_ID}`)
		return {
			success: true,
			client_id: process.env.GOOGLE_CLIENT_ID
		}
	} catch (err) {
		log.error(`Google config error: ${err.message}`)
...
export async function googleSignIn(request, reply) {
	try {
		...
		if (!access_token) {
			log.warn('Google sign-in attempted without access token')
		...
		if (!googleUser.id) {
			log.warn('Google sign-in failed: Invalid user data received')
		...
		log.info(`Google user attempting sign-in: ${name} (${email})`)
		...
		if (!user || user.anonymized_at) {
			if (user && user.anonymized_at)
				log.info(`Previously anonymized Google user ${googleId} creating new account`)
			...
			if (profilePictureUrl) {
				try {
					log.info(`Downloading Google profile picture for ${username}`)
					...
					if (imageResponse.ok) {
						...
						try {
							...
							log.info(`Google profile picture validated and saved: ${secureFilename}`);
						} catch (validationError) {
							log.warn(`Google profile picture validation failed: ${validationError.message}, using default`);
							profilePicture = "default-profile-picture.png";
						}
					} else {
						log.warn(`Failed to download Google profile picture: ${imageResponse.status}`)
					}
				} catch (error) {
					log.error(`Error downloading Google profile picture: ${error.message}`)
				}
			}
			
			const newUserInfo = await usersModel.createGoogleUser(username, hashedPassword, googleId, profilePicture)
			user = usersModel.getUserById(newUserInfo.lastInsertRowid)
			log.info(`New Google user created: ${username} (ID: ${user.userId})`)
		} else {
			log.info(`Existing Google user signing in: ${user.username} (ID: ${user.userId})`)
		}
		...
		if (isNewUser && tempPassword && email) {
            try {
				log.info(`📧 Sending welcome email to ${email} for user ${user.username}`)
                const emailSent = await sendWelcomeEmail(email, user.username, tempPassword)
                if (emailSent) {
                    log.info(`Welcome email sent successfully to ${email}`)
				} else {
					log.warn(`Welcome email failed to send to ${email}`)
				}
            } catch (emailError) {
                log.error(`Welcome email error: ${emailError.message}`)
            }
        }

		log.info(`Google sign-in successful for ${user.username}`)
		...
		log.error(`Critical Google sign-in error: ${error.message}`)
...
export async function getUserProfile(request, reply) {
	try {
		...
		if (!infos) {
			log.warn('Profile access denied: Unauthorized request')
		...
		if (!user) {
			log.warn('Profile access denied: User not found in token')
			return reply.code(401).send({ error: 'Unauthorized' })
		}

		const imgUrl = `uploads/${user.profile_picture}`
		log.info(`Profile retrieved for user: ${user.username}`)
		return reply.code(200).send({ success: true, user: user, accessToken: accessToken, profile_picture: imgUrl })
	} catch (error) {
		log.error(`Error retrieving user profile: ${error.message}`)
...
export async function getUserProfilePicture(request, reply) {
	try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('Profile picture access denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}
		...
		const accessToken = infos.accessToken
		if (!user) {
			log.warn('Profile picture access denied: User not found in token')
			return reply.code(401).send({ error: 'Unauthorized' })
		}

		const imgUrl = `uploads/${user.profile_picture}`
		log.debug(`Profile picture retrieved for user: ${user.username}`)
		return reply.code(200).send({ success: true, username: user.username, accessToken: accessToken, profile_picture: imgUrl })
	} catch (error) {
		log.error(`Error retrieving profile picture: ${error.message}`)
		reply.code(500).send({ error: 'Internal Server Error' })
	}
}
...
export async function createUser(request, reply) {
	...
	if (!username || !password) {
		log.warn('User creation failed: Missing username or password')
		return reply.code(400).send({ error: 'Username and Password are required' })
	}

	const usernameRegex = /^[A-Za-z0-9._-]+$/
	if (!usernameRegex.test(username)) {
		log.warn(`User creation failed: Invalid username format: ${username}`)
	...
	try {
		...
		log.error(`Critical error creating user ${username}: ${err.message}`)
...
export async function login(request, reply) {
	...
	try {
		...
		if (user.anonymized_at) {
			log.warn(`Login attempt on anonymized account: ${username}`)
		...
		log.error(`Critical login error for ${username}: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
export async function login1v1(request, reply) {
	...
	try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('1v1 login denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}
		...
		const accessToken = infos.accessToken
		if (!user || !accessToken) {
			log.warn('1v1 login denied: Invalid user or token')
			return reply.code(401).send({ error: 'Unauthorized' })
		}

		log.info(`1v1 opponent login attempt: ${username} (Player 1: ${user.username})`)
		const player2 = usersModel.getUserByUsername(username)
		if (!player2 || !await verifyPassword(player2.password, password)) {
			log.warn(`1v1 login failed: Invalid credentials for ${username}`)
			return reply.code(401).send({ success: false, error: 'Invalid credentials' })
		}

		if (player2.anonymized_at) {
			log.warn(`1v1 login failed: Anonymized account ${username}`)
			return reply.code(401).send({ success: false, error: 'This account has been deleted' })
		}

		usersModel.updateLastActivity(player2.userId)
		log.info(`1v1 game ready: ${user.username} vs ${player2.username}`)
		reply.code(200).send({ success: true, message: 'Opponent logged in', user: user, player2: player2, accessToken: accessToken })
	} catch (err) {
		log.error(`Critical error in 1v1 login: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
export async function login2v2(request, reply) {
	...
	try {
		...
		if (!player2 || !await verifyPassword(player2.password, password2)) {
			log.warn(`2v2 login failed: Invalid credentials for Player 2: ${username2}`)
		...
		if (!player3 || !await verifyPassword(player3.password, password3)) {
			log.warn(`2v2 login failed: Invalid credentials for Player 3: ${username3}`)
		...
		if (!player4 || !await verifyPassword(player4.password, password4)) {
			log.warn(`2v2 login failed: Invalid credentials for Player 4: ${username4}`)
		...
		log.info(`2v2 game ready: ${user.username} & ${player2.username} vs ${player3.username} & ${player4.username}`)
		reply.code(200).send({ 
			success: true, 
		...
		log.error(`Critical error in 2v2 login: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
export async function logout(request, reply) {
	...
	if (!accessToken || accessToken === "undefined") {
		log.warn('Logout attempt without valid access token')
	...
	try {
		...
		log.warn(`Failed to decode token during logout: ${decodeError.message}`)
	}
	...
	if (accessToken && accessToken !== "undefined") {
		...
		if (expiresIn > 0) {
			...
			log.debug(`Access token blacklisted for ${username}`)
		}
	}
	...
	if (refreshToken && refreshToken !== "undefined") {
		...
		if (expiresIn > 0) {
			...
			log.debug(`Refresh token blacklisted for ${username}`)
		}
...
export async function updateDoubleAuth(request, reply) {
	try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('Double auth update denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}

		const user = infos.user
		if (!user) {
			log.warn('Double auth update failed: User not found in token')
			return reply.code(404).send({ success: false, error: 'User not found' })
		}

		if (user.doubleAuth_status || user.doubleAuth_secret !== null) {
			log.info(`Disabling 2FA for user: ${user.username}`)
		...
		log.info(`Enabling 2FA for user: ${user.username}`)
		...
		log.error(`Critical error updating double auth: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
export async function accessProfileInfo(request, reply) {
	try {
		...
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('Access to profile info denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}
		...
		const accessToken = infos.accessToken
		if (!user) {
			log.warn('Access to profile info denied: User not found in token')
			return reply.code(404).send({ success: false, error: 'User not found' })
		}

		if (!await verifyPassword(user.password, password)) {
			log.warn(`Access to profile info denied: Invalid password for ${user.username}`)
			return reply.code(401).send({ success: false, error: 'Invalid password' })
		}
		
		log.info(`Access to profile info granted for user: ${user.username}`)
		return reply.code(200).send({success: true, accessToken: accessToken, message: 'Access to profile infos accepted', user: user})
	} catch (err) {
		log.error(`Critical error accessing profile info: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
export async function changeProfile(request, reply) {
	...
	try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('Profile update denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}
		...
		const accessToken = infos.accessToken
		if (!user) {
			log.warn('Profile update failed: User not found in token')
			return reply.code(404).send({ success: false, error: 'User not found' })
		}

		let updated = false
		if (newUsername) {
			const sameUsername = await usersModel.getUserByUsername(newUsername)
			if (sameUsername) {
				log.warn(`Profile update failed: Username already used - ${newUsername}`)
				return reply.code(409).send({ error: "This username is already used" })
			}
			log.info(`✏️ Updating username for user: ${user.username} to ${newUsername}`)
		...
		if (updated) {
			...
			log.info(`No changes made to profile for user: ${user.username}`)
			return reply.code(200).send({ success: true, message: 'No changes made' })
		}
	} catch (err) {
		log.error(`Critical error updating profile: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
async function validateImageFile(buffer, originalFilename) {
	...
	if (typeValidation.type === 'png' && !['.png'].includes(originalExt))
		log.warn(`File extension mismatch: ${originalExt} but detected PNG`);

	if (typeValidation.type === 'jpeg' && !['.jpg', '.jpeg'].includes(originalExt))
		log.warn(`File extension mismatch: ${originalExt} but detected JPEG`);
...
export async function changeProfilePicture(request, reply) {
	try {
		...
		if (!file) {
			log.warn('Profile picture update failed: No file uploaded')
			return reply.code(400).send({ 
				success: false, 
		...
		try {
			...
			log.info(`Image validation passed: ${imageValidation.type} format detected`);
		} catch (validationError) {
			log.warn(`Image validation failed: ${validationError.message}`);
			return reply.code(400).send({ 
				success: false, 
		...
		try {
			...
			log.info(`Profile picture saved: ${secureFilename}`);
		} catch (writeError) {
			log.error(`Error writing file: ${writeError.message}`);
		...
		if (oldProfilePicture !== "default-profile-picture.png") {
			try {
				...
				if (fileExists) {
					log.info(`🗑️ Deleting old profile picture: ${oldFilePath}`);
					await fs.unlink(oldFilePath);
				} else
					log.info(`Old profile picture doesn't exist: ${oldFilePath}`);
			} catch (deleteErr) {
		...
		log.info(`Profile picture updated successfully for user: ${user.username}`)
		return reply.code(200).send({
			success: true,
			accessToken: accessToken,
		...
		log.error("Error uploading new profile picture:", err);
...
export async function deleteAccount(request, reply) {
	try {
		...
		if (!infos) {
			log.warn('Account deletion denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}
		...
		const accessToken = infos.accessToken
		if (!user) {
			log.warn('Account deletion failed: User not found in token')
		...
		if (refreshToken && refreshToken !== undefined && refreshToken !== null) {
			...
			if (expiresInRefresh > 0) {
				...
				log.debug(`Refresh token blacklisted for account deletion: ${user.username}`)
			}
			reply.clearCookie('refreshToken', { path: '/' })
		}
		...
		if (accessToken && accessToken !== undefined && accessToken !== null) {
			...
			if (expiresInAccess > 0) {
				...
				log.debug(`Access token blacklisted for account deletion: ${user.username}`)
			}
		}
		...
		if (oldProfilePicture !== "default-profile-picture.png") {
			try {
				...
				if (fileExists) {
					log.info(`deleting old profile picture: ${oldFilePath}`)
					await fs.unlink(oldFilePath)
				} else {
					log.info(`Old profile picture doesn't exist: ${oldFilePath}`);
				}
		...
		if (info.changes === 0) {
			log.warn(`Account deletion failed: User not found - ${user.userId}`)
			return reply.code(404).send({ error: "User not found" })
		}

		log.info(`User ${user.username} (ID: ${user.userId}) has been anonymized`)
		return reply.send({ success: true, message: "Account anonymized successfully"})
	} catch (err) {
		log.error(`Critical error deleting account: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
...
export async function verifyDoubleAuth(request, reply) {
	try {
		...
		if (isValid) {
			...
			log.warn(`2FA verification failed: Invalid code for user ${user.username}`)
			return reply.code(401).send({ success: false, error: 'Invalid 2FA code' })
		}
	} catch (err) {
		log.error(`Critical error in 2FA verification: ${err.message}`)
...
export async function activateDoubleAuth(request, reply) {
	...
	const infos = await getUserFromToken(request)
	if (!infos) {
		log.warn('2FA activation denied: Unauthorized request')
		return reply.code(401).send({ success: false, error: 'Unauthorized' })
	}

	const user = infos.user
	if (!user) {
		log.warn('2FA activation failed: User not found in token')
	...
	if (isValid) {
		...
		log.info(`2FA successfully activated for user: ${user.username}`)
		return reply.send({ success: true, message: "2FA successfully activated" })
	} else {
		log.warn(`2FA activation failed: Invalid code for user ${user.username}`)
		return reply.code(400).send({ 
			success: false, 
...
export async function exportUserData(request, reply) {
	  try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('Data export denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}
		...
		const accessToken = infos.accessToken
		if (!user) {
			log.warn('Data export failed: User not found in token')
			return reply.code(401).send({ success: false , error: 'User not found' })
		}
		...
		log.info(`User data export successful for user: ${user.username}`)
		return reply.code(200).send(JSON.stringify({ success: true, ...exportData }, null, 2))
	} catch (error) {
		log.error(`Error exporting user data: ${error.message}`)
...
export async function anonymizeUser(request, reply) {
	try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('Anonymization denied: Unauthorized request')
			return reply.code(401).send({ success: false, error: 'Unauthorized' })
		}

		const user = infos.user
		if (!user) {
			log.warn('Anonymization failed: User not found in token')
			return reply.code(401).send({ success: false, error: 'User not found' })
		}
		...
		log.info(`Anonymizing user account: ${user.username}`)
		...
		log.info(`User account anonymized successfully: ${user.username}`)
		return reply.code(200).send({ success: true, message: 'User account anonymized successfully' })
	} catch (error) {
		log.error(`Error anonymizing user account: ${error.message}`)
