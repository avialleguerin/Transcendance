...
export async function getUserPlatformer(request, reply) {
	try {
		...
		if (!infos) {
			log.warn('Platformer access denied: Unauthorized request')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		let user = infos.user
		if (!user) {
			log.warn('Platformer access denied: User not found in token')
			return reply.code(401).send({ error: "User not found" })
		}
		if (!infos.accessToken) {
			log.warn('Platformer access denied: Missing access token')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		const platformer = platformersModel.getUserPlatformer(user.userId)
		log.success(`Platformer games retrieved for user: ${user.username} (${platformer.length} found)`)
		return reply.send({ success: true, user: user, games: platformer, accessToken: infos.accessToken })
	} catch (err) {
		log.error(`Error retrieving user platformer games: ${err.message}`)
...
export async function createPlatformer(request, reply) {
	...
	try {
		const infos = await getUserFromToken(request)
		if (!infos) {
			log.warn('Platformer creation denied: Unauthorized request')
			return reply.code(401).send({ error: "Unauthorized" })
		}
		const user = infos.user
		if (!user) {
			log.warn('Platformer creation failed: User not found in token')
			return reply.code(401).send({ error: "User not found" })
		}
		if (!infos.accessToken) {
			log.warn('Platformer creation denied: Missing access token')
			return reply.code(401).send({ error: "Unauthorized" })
		}

		if (!player1 || !player2 || score_player1 === undefined || score_player2 === undefined) {
			log.warn('Platformer creation failed: Missing parameters')
		...
		if (!user2) {
			log.warn(`Platformer creation failed: Player2 '${player2}' not found`)
			return reply.code(404).send({ success: false, error: `Player '${player2}' not found`, accessToken: infos.accessToken })
		}

		log.info(`Creating platformer game: ${player1} (${score_player1}) vs ${player2} (${score_player2})`)
		platformersModel.createPlatformer(user.userId, user2.userId, score_player1, score_player2)

		log.success(`Platformer game created successfully: ${player1} vs ${player2}`)
		...
		log.error(`Error creating platformer game: ${err.message}`)
		return reply.code(500).send({ error: err.message })
	}
}
