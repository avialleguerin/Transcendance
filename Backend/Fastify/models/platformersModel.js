...
export const CREATE_PLATFORMERS_TABLE = `
	CREATE TABLE IF NOT EXISTS platformers (
		...
		score_user1 INTEGER NOT NULL DEFAULT 0 CHECK(score_user1 >= 0),
		score_user2 INTEGER NOT NULL DEFAULT 0 CHECK(score_user2 >= 0),
...
const platformersModel = {
	createPlatformer: (user1_id, user2_id, score_user1, score_user2) => { db.prepare("INSERT INTO platformers (user1_id, user2_id, score_user1, score_user2) VALUES (?, ?, ?, ?)").run(user1_id, user2_id, score_user1, score_user2) },
	getAllPlatformers: () => db.prepare(`
		SELECT p.platformerId, p.user1_id, p.user2_id, p.score_user1, p.score_user2, p.created_at,
	...
	getUserPlatformer: (user) => { return db.prepare("SELECT p.platformerId, p.user1_id, p.score_user1, p.created_at, u.username FROM platformers p JOIN users u ON p.user1_id = u.userId WHERE p.user1_id = ? OR p.user2_id = ? ORDER BY p.created_at DESC").all(user, user) },
