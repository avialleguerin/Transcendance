...
const tokenModel = {
	blacklistAccessToken: (token) => db.prepare("INSERT INTO blacklist_access_tokens (token) VALUES (?)").run(token),
	blacklistRefreshToken: (token) => db.prepare("INSERT INTO blacklist_refresh_tokens (token) VALUES (?)").run(token),
	isAccessTokenBlacklisted: (token) => { return db.prepare("SELECT * FROM blacklist_access_tokens WHERE token = ?").get(token) !== undefined },
	isRefreshTokenBlacklisted: (token) => { return db.prepare("SELECT * FROM blacklist_refresh_tokens WHERE token = ?").get(token) !== undefined }
}
