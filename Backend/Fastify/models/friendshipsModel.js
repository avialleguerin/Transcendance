...
const friendshipsModel = {
	...
	getUserFriendships: (userId) => db.prepare(`SELECT f.friendshipId, f.userId, f.friendId, f.status, f.created_at, u.username as friend_username, u.profile_picture as friendProfilePicture FROM friendships f JOIN users u ON (CASE WHEN f.userId = ? THEN f.friendId = u.userId WHEN f.friendId = ? THEN f.userId = u.userId END) WHERE f.userId = ? OR f.friendId = ? ORDER BY f.created_at DESC`).all(userId, userId, userId, userId),
	createFriendship: (userId, friendId) => { return db.prepare("INSERT INTO friendships (userId, friendId) VALUES (?, ?)").run(userId, friendId) },
	acceptFriendRequest: (userId, friendId) => { return db.prepare("UPDATE friendships SET status = 'accepted' WHERE userId = ? AND friendId = ?").run(friendId, userId) },
	rejectFriendRequest: (userId, friendId) => { return db.prepare("UPDATE friendships SET status = 'rejected' WHERE userId = ? AND friendId = ?").run(friendId, userId) },
	...
	removeFriend: (userId, friendId) => { return db.prepare(`DELETE FROM friendships WHERE (userId = ? AND friendId = ?)  OR (userId = ? AND friendId = ?)`).run(userId, friendId, friendId, userId) },
