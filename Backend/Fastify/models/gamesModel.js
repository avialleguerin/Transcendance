...
const gamesModel = {
	createGame: async (user1_id, user2_id) => {
		const db = await getDb();
		db.prepare("INSERT INTO games (user1_id, user2_id) VALUES (?, ?)").run(user1_id, user2_id);
		return { user1_id, user2_id };
	},
	create1v1Game: async (user1_id, user2_id, score_left, score_user2) => {
		const db = await getDb();
	...
	create2v2Game: async (user1_id, user2_id, user3_id, user4_id, score_left, score_right) => {
		const db = await getDb();
	...
	updateScore: async (gameId, score_left, score_right) => {
		const db = await getDb();
		db.prepare("UPDATE games SET score_left = ?, score_user2 = ? WHERE gameId = ?").run(score_left, score_right, gameId);
	},
	deleteGame: async (gameId) => {
		const db = await getDb();
		return db.prepare("DELETE FROM games WHERE gameId = ?").run(gameId);
	},
	getGamesByUserId: async (userId) => {
		const db = await getDb();
		return db.prepare("SELECT * FROM games WHERE user1_id = ? OR user2_id = ? ORDER BY created_at DESC").all(userId, userId);
	}
}
