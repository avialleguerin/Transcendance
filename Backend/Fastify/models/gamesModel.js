...
const gamesModel = {
	...
	getUserGames: (user) => { return db.prepare("SELECT g.gameId, g.user1_id, g.user2_id, g.score_left, g.score_right, g.created_at, u1.username as user1_username, u1.profile_picture as user1ProfilePicture, u2.username as user2_username, u2.profile_picture as user2ProfilePicture FROM games g JOIN users u1 ON g.user1_id = u1.userId  JOIN users u2 ON g.user2_id = u2.userId WHERE g.user1_id = ? OR g.user2_id = ? ORDER BY g.created_at DESC").all(user, user) },
