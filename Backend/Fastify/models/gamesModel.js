...
const gamesModel = {
	...
	getUserGames: (user) => { return db.prepare("SELECT g.gameId, g.user1_id, g.user2_id, g.user3_id, g.user4_id, g.score_left, g.score_right, g.created_at, u1.username as user1_username, u1.profile_picture as user1ProfilePicture, u2.username as user2_username, u2.profile_picture as user2ProfilePicture, u3.username as user3_username, u3.profile_picture as user3ProfilePicture, u4.username as user4_username, u4.profile_picture as user4ProfilePicture FROM games g JOIN users u1 ON g.user1_id = u1.userId  JOIN users u2 ON g.user2_id = u2.userId LEFT JOIN users u3 ON g.user3_id = u3.userId LEFT JOIN users u4 ON g.user4_id = u4.userId WHERE g.user1_id = ? OR g.user2_id = ? OR g.user3_id = ? OR g.user4_id = ? ORDER BY g.created_at DESC").all(user, user, user, user) },
